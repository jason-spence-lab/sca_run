...
def write_rank_genes(adata, groupby, figdir='./figures'):
	...
	for cluster in adata.obs[groupby].cat.categories:
		csv_fileName = ''.join([figdir,'/csv_files/',cluster,'_clustercompare.csv'])
...
def find_genes(adata, gene_list, genes_exist=None, missing_genes=None):
	## Check inputs
	if not genes_exist:
		genes_exist = []
...
def pipe_basic(sca_dict, figdir='./figures/', load_save=None, new_save='adata_save.p'):
	...
	*Note that pickled adata files are very large - min 4GB (make sure to clean out unused pickle files)
	...
	return adata_dict#[adata,adata_preFiltered,adata_preProcessed,annotation_dict,summary_dict]
...
def pipe_ext(sca_dict, analysis_params_ext, figdir='./figures/', extracted=None, load_save=None):
	'''
